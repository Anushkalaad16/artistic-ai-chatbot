...
export default function ArtisticChatbot() {
  ...
  const iconButtonStyle = {
    backgroundColor: isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(255, 251, 240, 0.9)',
    border: `1px solid ${isDark ? 'rgba(255, 255, 255, 0.2)' : 'rgba(255, 109, 31, 0.2)'}`
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Logo */}
        <div className="flex items-center gap-3">
          <img src="/chaticon.png" alt="Artistic AI Logo" className="w-14 h-14 object-contain transition-all duration-300" />
          <h1 className="text-3xl font-black" style={{ color: theme.text }}>Artistic AI</h1>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            style={iconButtonStyle}
          >
            {isDark ? <Sun className="w-6 h-6" style={{ color: colors.orange }} /> : <Moon className="w-6 h-6" style={{ color: colors.orange }} />}
          </button>
        </div>
      </header>

      {/* Chat Messages */}
      <div className="flex-1 overflow-y-auto transparent-scrollbar px-8 py-6 flex flex-col items-center" style={{ scrollbarWidth: 'thin', scrollbarColor: 'transparent transparent' }}>
        <div className="w-full max-w-4xl space-y-6">
          {messages.map(message => (
            <div key={message.id} className={`flex items-start gap-4 ${message.isBot ? '' : 'flex-row-reverse'}`}>
              {message.isBot ? (
                <img src="/chaticon.png" alt="Artistic AI Logo" className="w-14 h-19 object-contain transition-all duration-300" />
              ) : (
                <div className="w-12 h-12 rounded-full flex items-center justify-center font-bold text-white text-lg flex-shrink-0" style={{ backgroundColor: colors.orange }}>Y</div>
              )}

              <div className={`flex flex-col ${message.isBot ? 'items-start' : 'items-end'} flex-1`}>
                <div className="px-8 py-3 rounded-[2rem] border-2 transition-all duration-300 hover:shadow-2xl max-w-2xl"
                  style={{
                    backgroundColor: message.isBot ? (isDark ? colors.dark.backgroundAlt : colors.white) : colors.darkGray,
                    borderColor: message.isBot ? (isDark ? 'transparent' : 'rgba(255, 109, 31, 0.1)') : 'transparent',
                    color: message.isBot ? theme.text : colors.white,
      ...
      {/* Input Area */}
      <div className="px-7 py-4 flex-shrink-0" style={{ borderColor: theme.border }}>
        <div className="max-w-4xl mx-auto rounded-full shadow-2xl flex items-center gap-4 px-8 py-2 border-2 transition-all duration-300 hover:shadow-[0_15px_60px_rgba(255,109,31,0.25)]"
          style={{ backgroundColor: isDark ? colors.dark.backgroundAlt : colors.white, borderColor: colors.orange, boxShadow: '0 10px 50px rgba(255, 109, 31, 0.15)' }}
        >
          <input
            type="text"
            value={inputValue}
          ...
          <button
            ...
            className="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 hover:rotate-12 active:scale-95 disabled:hover:scale-100 disabled:hover:rotate-0"
            style={{ 
              backgroundColor: inputValue.trim() && !loading ? colors.orange : (isDark ? colors.dark.border : colors.light.border),
              cursor: inputValue.trim() && !loading ? 'pointer' : 'not-allowed',
              opacity: inputValue.trim() && !loading ? 1 : 0.5
            }}
          >
            <Send size={20} style={{ color: inputValue.trim() && !loading ? colors.white : theme.textSecondary }} />
          </button>
        </div>
      </div>
    </div>
